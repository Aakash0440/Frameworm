...
class MemoryMonitor:
    ...
    def print_summary(self):
        ...
        for label, stats in self.snapshots.items():
            delta_str = f"+{stats['delta_gb']:.3f}" if stats['delta_gb'] >= 0 else f"{stats['delta_gb']:.3f}"
...
def estimate_model_memory(model: nn.Module, batch_size: int = 1, input_shape: tuple = None) -> Dict[str, float]:
    """
