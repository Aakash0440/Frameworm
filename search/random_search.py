...
class RandomSearch:
    ...
    def run(
        ...
        if n_jobs == 1:
            ...
            for i, config in enumerate(tqdm(configurations, desc="Random search") if verbose else configurations):
                result = self._evaluate_configuration(config, train_fn, i, experiment_root)
                results.append(result)
                
                # Check for improvement
                improved = (
                    (self.mode == 'min' and result['score'] < best_score_so_far) or
                ...
                # Early stopping
                if early_stopping_patience and trials_without_improvement >= early_stopping_patience:
                    if verbose:
