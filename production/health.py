        ...
        for name, check_fn in checks.items():
            ...
            try:
                ...
                if not isinstance(result, HealthStatus):
                    result = HealthStatus(
                        healthy=bool(result), message=str(result) if result else "Check passed"
                    )
            except Exception as e:
                result = HealthStatus(healthy=False, message=f"Check failed: {str(e)}")
...
def check_gpu_available() -> HealthStatus:
    ...
    if torch.cuda.is_available():
        ...
        return HealthStatus(healthy=False, message="No GPU available")
