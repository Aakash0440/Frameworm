...
async def root():
    """Root endpoint"""
    return {"message": "FRAMEWORM Dashboard API", "version": "1.0.0", "docs": "/docs"}


# Experiments API
@app.get("/api/experiments", response_model=List[ExperimentResponse])
async def list_experiments(
    status: Optional[str] = None, tags: Optional[List[str]] = Query(None), limit: int = 100
):
    """
...
async def export_model(request: ModelExportRequest):
    ...
    try:
        ...
        return {"success": True, "output_path": str(output_path), "format": request.format}
...
async def get_system_status():
    ...
    if torch.cuda.is_available():
        ...
        for i in range(torch.cuda.device_count()):
            ...
            status["gpus"].append(
                {"id": i, "name": torch.cuda.get_device_name(i), "memory_gb": mem}
            )
    else:
        status["gpu_count"] = 0
...
async def get_resource_usage():
    ...
    if torch.cuda.is_available():
        resources["gpus"] = []
        for i in range(torch.cuda.device_count()):
            from distributed import get_gpu_memory_usage
