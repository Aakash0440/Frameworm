...
class DriftResult:
    ...
    def __repr__(self):
        ...
        return f"DriftResult({status}, p={self.p_value:.4f}, " f"test={self.test_name})"
...
class DriftDetector:
    ...
    def check(self, production: np.ndarray, return_per_feature: bool = False) -> DriftResult:
        """
    ...
    def _ks_test(self, production: np.ndarray) -> DriftResult:
        ...
        for i in range(n_features):
            _, p = stats.ks_2samp(self.reference_data[:, i], production[:, i])
            p_values.append(p)
