...
class InMemoryMetrics:
    ...
    def __init__(self, history_size: int = 1000):
        ...
        self._data: Dict[str, deque] = defaultdict(lambda: deque(maxlen=history_size))
        self._timestamps: Dict[str, deque] = defaultdict(lambda: deque(maxlen=history_size))
...
class TrainingMetricsCollector:
    ...
    def __init__(self, namespace: str = "frameworm", use_prometheus: bool = True):
    ...
    def print_summary(self):
        ...
        for name in key_metrics:
            ...
            if summary:
                print(
                    f"  {name:<25} latest={summary['latest']:.4f}  "
    ...
    def __init__(self, namespace: str = "frameworm"):
        ...
        if PROMETHEUS_AVAILABLE:
            ...
            self.prom_throughput = Gauge(
                f"{namespace}_inference_throughput", "Inference throughput (requests/sec)"
            )
...
class MetricsExporter:
    ...
    def start(self):
        ...
        self._server_thread = threading.Thread(
            target=lambda: start_http_server(self.port), daemon=True
        )
