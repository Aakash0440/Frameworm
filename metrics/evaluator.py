...
class MetricEvaluator:
    ...
    def generate_samples(
        self, model: torch.nn.Module, num_samples: int, generation_fn: Optional[Callable] = None
    ) -> torch.Tensor:
        """
        ...
        with torch.no_grad():
            for i in tqdm(range(num_batches), desc="Generating samples"):
                ...
                if generation_fn is not None:
                    ...
                    raise ValueError("Model must have .sample() method or provide generation_fn")
    ...
    def get_real_images(self, num_samples: Optional[int] = None) -> torch.Tensor:
        """
    ...
    def evaluate(
        ...
        if "fid" in self.metric_objects:
            ...
            fid_score = self.metric_objects["fid"].compute(
                real_images, generated_images, show_progress=True
            )
    ...
    def evaluate_checkpoint(
        self, checkpoint_path: str, model_class: type, config: Any, **kwargs
    ) -> Dict[str, float]:
        """
...
def quick_evaluate(
    model: torch.nn.Module, real_data: Any, num_samples: int = 5000, device: str = "cuda"
) -> Dict[str, float]:
    """
    ...
    Returns:
        Metric results
    """
    evaluator = MetricEvaluator(metrics=["fid", "is"], real_data=real_data, device=device)
