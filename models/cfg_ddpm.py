...
class CFGUNet(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, t: torch.Tensor, class_label: Optional[torch.Tensor] = None
    ) -> torch.Tensor:
        """
...
class CFGDDPM(nn.Module):
    ...
    def compute_loss(
        self, x0: torch.Tensor, y: Optional[torch.Tensor] = None
    ...
    def sample(
        ...
        """
        x = torch.randn(
            num_samples, self.image_channels, self.image_size, self.image_size, device=device
        )
        ...
        if class_label is not None:
            ...
            null_labels = torch.full(
                (num_samples,), self.num_classes, dtype=torch.long, device=device
            )
