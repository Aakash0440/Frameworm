...
class Encoder(nn.Module):
    ...
    def __init__(self, channels, latent_dim, input_size=32):
        ...
        self.conv_layers = nn.Sequential(
            nn.Conv2d(channels, 32, 4, 2, 1),  # stride 2, kernel 4
            nn.ReLU(),
    ...
    def _get_conv_output_size(self):
        ...
        for _ in range(4):
            size = (size + 2*1 - 4)//2 + 1  # Conv formula: (W + 2P - K)//S + 1
...
class Decoder(nn.Module):
    ...
    def __init__(self, latent_dim, channels, output_size=32):
        ...
        # Compute initial size for deconv
        size = output_size
        for _ in range(4):
            size = (size + 1) // 2  # approximate reverse of conv downsampling
