...
class VQDecoder(nn.Module):
    ...
    def __init__(
        ...
        # Upsampling
        layers.append(
            nn.ConvTranspose2d(hidden_channels, hidden_channels, 4, stride=upsample, padding=1)
        )
        layers.append(nn.ReLU(inplace=True))
...
class VQVAE2(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.top_to_bottom = nn.Sequential(
            nn.ConvTranspose2d(emb_dim, emb_dim, 4, stride=4, padding=0), nn.ReLU(inplace=True)
        )
    ...
    def decode(
        self, z_q_top: torch.Tensor, z_q_bottom: torch.Tensor, target_size=None
    ) -> torch.Tensor:
