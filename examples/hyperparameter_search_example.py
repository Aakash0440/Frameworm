...
def example_grid_search():
    ...
    # Define search space
    search_space = {"training.lr": [0.001, 0.0005, 0.0001], "training.batch_size": [64, 128, 256]}

    # Create search
    search = GridSearch(
        base_config=base_config, search_space=search_space, metric="val_loss", mode="min"
    )
    ...
    best_config, best_score = search.run(
        train_fn=train_vae, n_jobs=1, experiment_root="experiments/grid_search", verbose=True
    )
...
def main():
    ...
    if random_best_score < grid_best_score:
        ...
        print(
            f"  Improvement: {(grid_best_score - random_best_score) / grid_best_score * 100:.1f}%"
        )
    else:
