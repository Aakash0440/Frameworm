...
def compare_search_methods():
    ...
    best_config, best_score = grid_search.run(
        ...
        experiment_root='experiments/comparison/grid',
        verbose=True
    )
    ...
    best_config, best_score = random_search.run(
        train_fn=train_vae_quick,
        experiment_root='experiments/comparison/random',
        verbose=True
    )
    ...
    for method, data in results.items():
        print(f"{method:<25} {data['best_score']:<12.4f} {data['time']:<10.1f} {data['n_trials']:<8}")
