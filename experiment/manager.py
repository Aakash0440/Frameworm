...
class ExperimentManager:
    ...
    def get_experiment(self, experiment_id: str) -> Dict[str, Any]:
        ...
        # Get config
        cursor.execute(
            "SELECT config_key, config_value FROM configs WHERE experiment_id = ?",
            (experiment_id,)
        )
    ...
    def compare_experiments(
        ...
        for exp_id in experiment_ids:
            ...
            if metrics:
                metric_filter = " AND metric_name IN ({})".format(
    ...
    def delete_experiment(self, experiment_id: str):
        ...
        cursor.execute(
            "SELECT path FROM experiments WHERE experiment_id = ?",
            (experiment_id,)
        )
    ...
    def search_experiments(
        ...
        if config_filter:
            for i, (key, value) in enumerate(config_filter.items()):
                alias = f"c{i}"
                joins.append(
                    f"JOIN configs {alias} ON e.experiment_id = {alias}.experiment_id"
                )
                where_clauses.append(
                    f"({alias}.config_key = ? AND {alias}.config_value = ?)"
                )
        ...
        if metric_filter:
            for i, (metric_name, (operator, value)) in enumerate(metric_filter.items()):
                alias = f"m{i}"
                joins.append(
                    f"JOIN metrics {alias} ON e.experiment_id = {alias}.experiment_id"
                )
                where_clauses.append(
