...
for name, step, loss, grad in failure_types:
    ...
    reason = []
    if loss_anomaly:
        reason.append(f"loss_z={z}")
    if grad_explode:
        reason.append(f"grad_explosion={grad}")
    if grad_vanish:
        reason.append(f"grad_vanishing={grad}")
    if mode_collapse:
        reason.append("mode_collapse")

    results.append({"name": name, "loss": loss, "detected": detected, "z_score": z})
    print(
        f"{name:<25} loss={loss:<8} detected={str(detected):<6} z={z} | {', '.join(reason) or 'none'}"
    )
