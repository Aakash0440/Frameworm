...
class SmallVAE(nn.Module):
    ...
    def decode(self, z): return self.dec_net(z)
...
def train_one(model, loader, optimizer, inject, epoch, is_shadow=False):
    ...
    for (batch,) in loader:
        ...
        if not torch.isfinite(l):
            losses.append(9999.0); gns.append(0.0)
    ...
    return (float(np.mean(losses)) if losses else 9999.0,
            float(np.mean(gns))   if gns   else 0.0)
...
def run_experiment(eid, name, lr, epochs, inject, agent_on, dataset, shadow_ds):
    ...
    for ep in range(1, epochs + 1):
        ...
        if agent_on and sentinel_streak >= 2:
            ...
            interventions.append({"epoch": ep, "anomaly": "MODEL_DEAD",
                                   "new_lr": cur_lr})
            sentinel_streak = 0
            window = SignalWindow()
        ...
        if agent_on and anomaly not in ("HEALTHY", "N/A"):
            ...
            if action == "REINIT" and plateau_reinits < 2:
                ...
                plateau_reinits += 1
                window           = SignalWindow()
                interventions.append({"epoch": ep, "anomaly": anomaly,
                                      "new_lr": cur_lr, "action": "REINIT"})
                ...
                for pg in optimizer.param_groups:
                    pg["lr"] = new_lr
                interventions.append({"epoch": ep, "anomaly": anomaly,
                                      "new_lr": new_lr, "action": "ADJUST_LR"})
                print(f"  [AGENT] ep{ep} {anomaly} => LR->{new_lr:.2e}")

        history.append({"epoch": ep, "loss": round(ep_loss, 4),
                        "gn": round(ep_gn, 4), "anomaly": anomaly})
        print(f"  epoch {ep:02d}/{epochs}  loss={ep_loss:.4f}  "
              f"gn={ep_gn:.3f}  [{anomaly}]")
    ...
    return {
        "exp_id": eid, "name": name, "lr": lr,
        "inject_anomaly": inject or "none",
        "final_loss": agent_loss, "shadow_final_loss": shadow_loss,
        "loss_delta": delta,
        "n_anomalies": len(anomalies), "anomalies": anomalies,
        "n_interventions": len(interventions), "interventions": interventions,
        "agent_resolved": resolved, "history": history,
...
def print_table(results):
    ...
    for r in results:
        cond = (r["inject_anomaly"] if r["inject_anomaly"] != "none"
    ...
    lines += [
        ...
        "  SUMMARY",
        (f"  Baseline avg loss:              "
         f"{np.mean([r['final_loss'] for r in baseline]):.4f}") if baseline else "",
        f"  Runs with anomalies detected:   {len(anom_r)} / {len(results)}",
        f"  Anomalies resolved by agent:    {len(resol_r)} / {len(anom_r)}",
        f"  Resolution rate:                "
        f"{len(resol_r) / max(len(anom_r), 1) * 100:.1f}%",
        f"  Mean loss delta (agent-shadow): "
        f"{np.mean([r['loss_delta'] for r in results]):+.4f}",
...
def save_db(results):
    ...
    for r in results:
        conn.execute("INSERT INTO results VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)", (
            r["exp_id"], r["name"], r["lr"], r["inject_anomaly"],
            r["final_loss"], r["shadow_final_loss"], r["loss_delta"],
            r["n_anomalies"], r["n_interventions"], int(r["agent_resolved"]),
            r["elapsed_s"], r["timestamp"], json.dumps(r)))
