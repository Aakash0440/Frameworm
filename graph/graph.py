...
class Graph:
    ...
    def get_node(self, node_id: str) -> Node:
        ...
        if node_id not in self.nodes:
            ...
            raise KeyError(f"Node '{node_id}' not found. Available: {available}")
    ...
    def remove_node(self, node_id: str):
        ...
        dependents = [n_id for n_id, node in self.nodes.items() if node_id in node.depends_on]
    ...
    def get_dependents(self, node_id: str) -> List[str]:
        ...
        return [n_id for n_id, node in self.nodes.items() if node_id in node.depends_on]
        ...
        for node_id, node in self.nodes.items():
            for dep_id in node.depends_on:
                if dep_id not in self.nodes:
                    raise GraphError(f"Node '{node_id}' depends on '{dep_id}' which doesn't exist")
                in_degree[node_id] += 1
    ...
    def execute_parallel(
        ...
        **kwargs,
        ...
        Returns:
            Dictionary of results
        """
        engine = ParallelExecutionEngine(self, max_workers=max_workers, executor_type=executor_type)

        return engine.execute(
            initial_inputs=initial_inputs, progress_callback=progress_callback, **kwargs
        )
    ...
    def execute(
        ...
        for node_id in execution_order:
            ...
            skipped_deps = [
                dep
                for dep in node.depends_on
                if self.graph.get_node(dep).status == NodeStatus.SKIPPED
            ]

            if failed_deps:
                node.status = NodeStatus.SKIPPED
                self.errors[node_id] = GraphError(f"Dependencies failed: {failed_deps}")
            ...
            try:
                ...
                if not continue_on_error:
                    raise GraphError(
                        f"Node '{node_id}' failed: {e}", node_id=node_id, original_error=e
                    )

        return self.results

    def _prepare_inputs(self, node: Node, initial_inputs: Dict[str, Any]) -> Dict[str, Any]:
        """
...
class CachedGraph(Graph):
    ...
    def execute(
        self, initial_inputs: Optional[Dict[str, Any]] = None, use_cache: bool = True, **kwargs
    ) -> Dict[str, Any]:
            ...
            with ExecutorClass(max_workers=min(self.max_workers, len(level_nodes))) as executor:
                ...
                for node_id in level_nodes:
                    ...
                    if self.executor_type == "process":
                        ...
                        future = executor.submit(_execute_node_process, node_id, node.fn, inputs)
                    else:
                        # Thread mode can use bound method safely
                        future = executor.submit(self._execute_node_safe, node_id, inputs)
