...
def test_server_builder_generates_file():
    ...
    with tempfile.TemporaryDirectory() as d:
        path = builder.build(
            model_type="dcgan", model_name="test_gan",
            model_version="v1.0", model_path="model/test.pt",
            output_dir=d, port=8001,
        )
...
def test_server_builder_all_model_types():
    ...
    with tempfile.TemporaryDirectory() as d:
        for model_type in MODEL_SPECS:
            ...
            path = builder.build(
                model_type=model_type, model_name=f"test_{model_type}",
                model_version="v0.1", model_path="model/m.pt",
                output_dir=out,
            )
...
def test_server_builder_shift_injection():
    ...
    with tempfile.TemporaryDirectory() as d:
        path = builder.build(
            model_type="vae", model_name="vae_test",
            model_version="v1", model_path="m.pt",
            output_dir=d, shift_reference="my_reference",
        )
        code = open(path).read()
...
def test_dockerfile_generated():
    ...
    with tempfile.TemporaryDirectory() as d:
        path = builder.generate_dockerfile(
            server_dir=d, model_name="test", model_version="v1",
            model_path="model/test.pt", port=8080,
        )
        assert os.path.exists(path)
...
def test_docker_compose_generated():
    ...
    with tempfile.TemporaryDirectory() as d:
        path = DockerBuilder().generate_compose(
            model_name="mymodel", model_version="v2",
            port=9000, output_path=os.path.join(d, "docker-compose.yml"),
        )
        assert os.path.exists(path)
        content = open(path).read()
