...
def print_banner():
    ...
    console.print(Align.center(f"[{CD}]Production-grade generative AI framework  ·  v{VERSION}  ·  github.com/aakash0440/frameworm[/{CD}]"))
    console.print()
...
def success(msg): console.print(f"[{G}]✓[/{G}]  [{C}]{msg}[/{C}]")
def error(msg):   console.print(f"[{R}]✗[/{R}]  [{C}]{msg}[/{C}]")
def info(msg):    console.print(f"[{A}]·[/{A}]  [{CD}]{msg}[/{CD}]")
...
def status():
    ...
    info_t.add_row(f"[{CD}]Python  [{C}]{platform.python_version()}[/{C}]", f"[{CD}]Platform  [{C}]{platform.system()}[/{C}]")
    try:
        import torch
        info_t.add_row(f"[{CD}]PyTorch  [{C}]{torch.__version__}[/{C}]", f"[{CD}]CUDA  [{C}]{'✓ '+torch.version.cuda if torch.cuda.is_available() else '✗ CPU only'}[/{C}]")
    except ImportError:
...
def shell():
    ...
    print_banner()
    console.print(Panel(f"[{CD}]Type a command or [bold]help[/bold]. [bold]exit[/bold] to quit.[/{CD}]", border_style=OD, padding=(0,2)))
    console.print()
    while True:
        try:
        ...
        if cmd in ("exit","quit","q"):
            console.print(f"[{CD}]Goodbye.[/{CD}]"); break
        if cmd == "help":
            print_help_menu(); continue
        try:
...
def train(config, resume, gpus):
    ...
    try:
        ...
        error(f"Model load failed: {e}"); return
    device = "cuda" if torch.cuda.is_available() else "cpu"
    info(f"Device: {device}")
    optimizer = torch.optim.Adam(model.parameters(), lr=cfg.get("optimizer.lr", 0.0002))
    trainer = Trainer(model=model, optimizer=optimizer, device=device, checkpoint_dir="checkpoints",
...
def serve(checkpoint, port, host):
    ...
    try:
        ...
        def generate(latent_dim:int=100):
            z = torch.randn(1,latent_dim); return {"shape":list(z.shape)}
    ...
    except ImportError: error("Missing: pip install fastapi uvicorn[standard]")
...
def evaluate(config, checkpoint, metrics, num_samples):
    ...
    run_evaluation(config_path=config, checkpoint_path=checkpoint,
                   metrics=metrics.split(","), num_samples=num_samples)
...
def export(checkpoint, format, output, quantize, benchmark):
    ...
    export_model(checkpoint_path=checkpoint, export_format=format,
                 output_dir=output, quantize=quantize, benchmark=benchmark)
...
def search(config, space, method, trials, parallel):
    ...
    run_search(config_path=config, space_path=space, method=method, n_trials=trials, n_jobs=parallel)
...
def cost_estimate(arch, hardware, latency, batch):
    ...
    if c.optimization_hint:
        console.print(Panel(f"[{A}]{c.optimization_hint}[/{A}]", title=f"[{O}]💡 Hint[/{O}]", border_style=OD))
...
def cost_compare(latency, hardware):
    ...
    for r in results:
        ...
        t.add_row(r.architecture, f"[{color}]${r.total_cost_usd:.8f}[/{color}]",
                  f"${r.cost_per_1k_requests:.4f}", f"${monthly:,.2f}", f"[{color}]{ratio:.1f}x[/{color}]")
    console.print(t)
...
def init(project_name, template, path):
    ...
    with console.status(f"[{A}]Creating {project_name}...[/{A}]", spinner="dots"):
        ...
        _yaml.dump({
            "model":    {"type": template if template!="basic" else "dcgan","latent_dim":100,"image_size":64,"channels":3,"features_g":64,"features_d":64},
            "training": {"epochs":100,"batch_size":64,"gradient_accumulation_steps":1},
            "optimizer":{"lr":0.0002,"beta1":0.5,"beta2":0.999},
        }, open(project_path/"configs"/"config.yaml","w"), default_flow_style=False)
    t = Table(box=box.SIMPLE, show_header=False, padding=(0,1))
    t.add_row(f"[{O}]{project_name}/[/{O}]", "")
    for d in dirs: t.add_row(f"[{CD}]  ├── {d}/[/{CD}]","")
...
def list_plugins(verbose):
    ...
    if verbose: loader.print_plugins(); return
    ps = loader.list_plugins()
    if not ps: info("No plugins found."); return
    t = Table(box=box.SIMPLE, show_header=True, header_style=f"bold {CD}", padding=(0,2))
    t.add_column("Status",width=6); t.add_column("Name",style=f"{O}",width=20)
    t.add_column("Version",width=8); t.add_column("Description",style=f"{CD}")
    for name,meta in ps.items():
        t.add_row(f"[{G}]✓[/{G}]" if loader._loaded.get(name) else f"[{CD}]○[/{CD}]",
...
def unload_plugin_cmd(plugin_name):
    """Unload a plugin."""
    from plugins.loader import get_plugin_loader
    get_plugin_loader().unload(plugin_name); success(f"Unloaded: {plugin_name}")
...
def plugin_info(plugin_name):
    """Show plugin details."""
    from plugins.loader import get_plugin_loader
    loader = get_plugin_loader(); ps = loader.list_plugins()
    if plugin_name not in ps: error(f"Plugin '{plugin_name}' not found"); return
    meta = ps[plugin_name]
    t = Table(box=box.SIMPLE, show_header=False, padding=(0,2))
    for k,v in [("Name",meta.name),("Version",meta.version),("Author",meta.author),
                ("Description",meta.description),("Hooks",", ".join(meta.hooks))]:
...
def create_plugin(name,dir):
    ...
    import yaml as _yaml
    plugin_dir = Path(dir)/name; plugin_dir.mkdir(parents=True,exist_ok=True)
    _yaml.dump({"name":name,"version":"0.1.0","author":"Your Name","description":f"{name} plugin",
                "entry_point":"plugin:register","dependencies":["torch>=2.0.0"],"hooks":["model"]},
               open(plugin_dir/"plugin.yaml","w"))
    open(plugin_dir/"__init__.py","w").write(f'def register():\n    print("✓ {name} registered")\n')
...
@config.command("list")
def config_list():
    configs = [c for c in list(Path(".").rglob("*.yaml"))+list(Path(".").rglob("*.yml")) if "venv" not in str(c)]
    if not configs: info("No config files found."); return
    t = Table(box=box.SIMPLE, show_header=False, padding=(0,2))
    for c in configs[:20]: t.add_row(f"[{O}]{c}[/{O}]")
...
def config_validate(config_file):
    from core import Config
    try: Config(config_file); success(f"Valid: {config_file}")
    except Exception as e: error(f"Invalid: {e}"); sys.exit(1)
