...
class TrainingError(FramewormError): pass


class Trainer:
    """Trainer for PyTorch models with callbacks, EMA, mixed precision, and checkpointing."""
    def __init__(self, model: nn.Module, optimizer: Optimizer, criterion: Optional[Callable] = None,
                 device: str = 'cuda', checkpoint_dir: str = 'checkpoints',
                 gradient_accumulation_steps: int = 1, log_every_n_steps: int = 100):
        self.model = model
    ...
    # ------------------ Setup Methods ------------------
    def set_scheduler(self, scheduler: _LRScheduler): self.scheduler = scheduler
    ...
    def enable_ema(self, decay: float = 0.999): self.ema = EMAModel(self.model, decay)
    ...
    def set_experiment(self, experiment: 'Experiment'):
        ...
        if experiment: self.checkpoint_dir = experiment.checkpoint_dir

    def set_early_stopping(self, patience: int, min_delta: float = 0.0):
        self.early_stopping_patience = patience
        self.early_stopping_min_delta = min_delta

    def add_callback(self, callback: Callback): self.callbacks.append(callback)
    def add_logger(self, logger: Logger): self.loggers.append(logger)

    def set_evaluator(self, evaluator: 'MetricEvaluator', eval_every: int = 5):
        """Attach automatic evaluation callback"""
    ...
    def train(self, train_loader: DataLoader, val_loader: Optional[DataLoader] = None,
              epochs: int = 100, resume_from: Optional[str] = None):
        ...
        try:
            for epoch in range(self.state.current_epoch, epochs):
                ...
                if self.experiment:
                    self.experiment.log_metrics(train_metrics, epoch=epoch, step=self.state.global_step, metric_type='train')
                    if val_metrics:
                        self.experiment.log_metrics(val_metrics, epoch=epoch, step=self.state.global_step, metric_type='val')

                # Best epoch saving
                if val_metrics and 'loss' in val_metrics:
                    is_best = self.state.is_best_epoch(val_metrics['loss'], mode='min')
                    if is_best: self.save_checkpoint('best.pt', epoch)
                ...
                if self.scheduler:
                    if isinstance(self.scheduler, torch.optim.lr_scheduler.ReduceLROnPlateau):
                        if val_metrics and 'loss' in val_metrics: self.scheduler.step(val_metrics['loss'])
                    else:
                        self.scheduler.step()
    ...
    def train_epoch(self, train_loader, epoch):
        ...
        if self.gradient_accumulator: self.gradient_accumulator.reset()

        for batch_idx, batch in enumerate(train_loader):
            batch = [b.to(self.device) if isinstance(b, torch.Tensor) else b for b in batch] \
                if isinstance(batch, (list, tuple)) else batch.to(self.device) if isinstance(batch, torch.Tensor) else batch
            ...
            if self.mixed_precision:
                with torch.cuda.amp.autocast():
                    ...
                    loss = loss_dict['loss']
                    if self.gradient_accumulator: loss = self.gradient_accumulator.scale_loss(loss)
                self.grad_scaler.scale(loss).backward()
                if self.gradient_clipper:
                    self.grad_scaler.unscale_(self.optimizer)
                    loss_dict['grad_norm'] = self.gradient_clipper.clip(self.model.parameters())
                should_update = (not self.gradient_accumulator or self.gradient_accumulator.should_update())
                if should_update:
                ...
                if self.gradient_accumulator: loss = self.gradient_accumulator.scale_loss(loss)
                loss.backward()
                if self.gradient_clipper: loss_dict['grad_norm'] = self.gradient_clipper.clip(self.model.parameters())
                should_update = (not self.gradient_accumulator or self.gradient_accumulator.should_update())
                if should_update:
                    self.optimizer.step()
                    if self.ema: self.ema.update()
    ...
    def _compute_loss(self, batch):
        ...
        if self.criterion is None: raise ValueError("No criterion provided and model has no compute_loss")
    ...
    def validate_epoch(self, val_loader: DataLoader, epoch: int) -> Dict[str, float]:
        ...
        with torch.no_grad():
            for batch in val_loader:
                ...
                if hasattr(self.model, 'compute_loss'):
                    loss_dict = self.model.compute_loss(*batch) if isinstance(batch, (list, tuple)) else self.model.compute_loss(batch)
                    metrics = loss_dict
                else:
                    if isinstance(batch, (list, tuple)):
                        *inputs, targets = batch
    ...
    def save_checkpoint(self, filename, epoch):
        ...
        if self.grad_scaler: checkpoint['grad_scaler_state_dict'] = self.grad_scaler.state_dict()
        torch.save(checkpoint, self.checkpoint_dir / filename)

    def _should_stop_early(self) -> bool:
        if self.early_stopping_patience is None: return False
