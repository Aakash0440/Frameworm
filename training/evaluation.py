...
class EvaluationCallback:
    ...
    def __init__(self, evaluator: MetricEvaluator, eval_every: int = 5, num_samples: int = 5000):
    ...
    def on_epoch_end(self, epoch: int, metrics: Dict[str, float], trainer):
        ...
        # Evaluate
        results = self.evaluator.evaluate(trainer.model, num_samples=self.num_samples)

        # Log to experiment if available
        if trainer.experiment:
            for metric_name, value in results.items():
                trainer.experiment.log_metric(
                    f"eval_{metric_name}", value, epoch=epoch, metric_type="eval"
                )
