...
class Callback:
    ...
    def on_batch_begin(self, batch_idx: int, trainer): pass
    def on_batch_end(self, batch_idx: int, metrics: Dict[str, float], trainer): pass
...
class CallbackList:
    ...
    def on_train_begin(self, trainer):
        for callback in self.callbacks: callback.on_train_begin(trainer)

    def on_train_end(self, trainer):
        for callback in self.callbacks: callback.on_train_end(trainer)

    def on_epoch_begin(self, epoch: int, trainer):
        for callback in self.callbacks: callback.on_epoch_begin(epoch, trainer)

    def on_epoch_end(self, epoch: int, metrics: Dict[str, float], trainer):
        for callback in self.callbacks: callback.on_epoch_end(epoch, metrics, trainer)

    def on_batch_begin(self, batch_idx: int, trainer):
        for callback in self.callbacks: callback.on_batch_begin(batch_idx, trainer)

    def on_batch_end(self, batch_idx: int, metrics: Dict[str, float], trainer):
        for callback in self.callbacks: callback.on_batch_end(batch_idx, metrics, trainer)
...
class ModelCheckpoint(Callback):
    ...
    def on_epoch_end(self, epoch: int, metrics: Dict[str, float], trainer):
        ...
        improved = (current_value < self.best_value) if self.mode == 'min' else (current_value > self.best_value)
...
class GradientMonitor(Callback):
    ...
    def on_batch_end(self, batch_idx: int, metrics: Dict[str, float], trainer):
        if batch_idx % self.log_every_n_batches != 0: return
        total_norm = 0.0
