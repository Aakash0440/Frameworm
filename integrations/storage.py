...
class GCSStorage(Callback):
    ...
    def __init__(self, bucket: str, prefix: str = "", credentials_path: Optional[str] = None):
        try:
            from google.cloud import storage
        except ImportError:
            raise ImportError(
                "google-cloud-storage not installed. " "Install: pip install google-cloud-storage"
            )
...
class AzureStorage(Callback):
    ...
    def __init__(
        self, account_name: str, container: str, prefix: str = "", account_key: Optional[str] = None
    ):
        try:
            from azure.storage.blob import BlobServiceClient
        except ImportError:
            raise ImportError(
                "azure-storage-blob not installed. " "Install: pip install azure-storage-blob"
            )
...
def setup_azure_storage(
    account_name: str, container: str, prefix: str = "", **kwargs
) -> AzureStorage:
    """Quick Azure setup"""
